...
## 2023-10-27 - Optimize player hand dev cards iteration
**Learning:** React component renders often involve iteration over collections inside `useMemo` hooks. If you have arrays, running multiple nested `.filter()` iterations inside a loop (O(N*M)) over small data sets can be easily bottlenecked. By replacing `.filter()` with O(N) one-pass dynamic object or map updates you can achieve 2x to 5x performance gains. We should avoid hardcoding type lists (`knight`, `yearOfPlenty`, etc.) in switch cases as this reduces maintainability in exchange for minimal performance gain compared to dynamic Object.keys/value lookup which avoids hardcoding.
**Action:** When optimizing loop bottlenecks involving `.filter`, restructure to do a single pass over the array tracking counts in a Map/object, then map those tallied results to the target structure.
