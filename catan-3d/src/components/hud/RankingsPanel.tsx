...
export function RankingsPanel({
  ...
  return (
    <div className="pointer-events-auto absolute top-[23rem] left-4 flex w-52 flex-col gap-2 rounded-2xl border border-glass-border bg-glass p-3 shadow-[0_8px_30px_rgba(0,0,0,0.35)] backdrop-blur-xl">
      <span className="px-1 font-body text-[10px] tracking-[0.25em] text-white/50 uppercase">Rankings</span>
      <div className="flex flex-col gap-1">
        {ranked.map(({ player, score, hidden }, index) => (
          <div key={player.id} className="flex items-center justify-between rounded-full bg-white/5 py-1 pr-2.5 pl-1.5">
