...
describe('calculateLongestRoad', () => {
  ...
  it('is not blocked by an unowned empty knight vertex', () => {
    const edges = chain('A', 'B', 'C')
    expect(calculateLongestRoad(1, ownedBy(1, edges), graphOf(edges), {}, new Map([['B', undefined as unknown as Building]]))).toBe(2)
  })

  it('evaluates adjacency without crashing on disconnected vertices', () => {
    const edges = chain('A', 'B', 'C')
    // Pass an empty adjacency to dfs by ensuring it checks an isolated component
    const extraGraph = { edges: [...edges, edge('E', 'X', 'Y')] } as BoardGraph
    const roads = ownedBy(1, edges) // player doesn't own E, so adjacency.get('X') is undefined
