...
3. **56 of 57 `channel.on<T>(...)` subscriptions** are the identical shape: register an event, forward its payload to `handlersRef.current.onX?.(payload)`. Also exhaustively checked — exactly one exception, `onGameStarted`, which calls its handler with multiple destructured arguments instead of a single payload object (matching `broadcastGameStarted`'s own exception above) — excluded from the generalization by design, not an oversight.
...
**Deviation, Sub-plan 1 (already reviewed and approved):** `send<T>`/`forwardTo<T>` shipped with no dedicated unit tests, deviating from "a small number of unit tests per helper" above. Rationale, confirmed sound by that sub-plan's own final whole-branch review: `useRoomChannel.ts` has no test file anywhere in this codebase (consistent precedent — no Supabase-channel-wiring code is unit-tested here), and the sub-plan's actual verification (`npx tsc -p tsconfig.app.json`, `eslint`, the full existing suite, `npm run build`, plus an exhaustive content-based exception check) already caught the one real bug that existed (the `forwardTo` type-safety gap — found by the review's own hand-tracing, not by a unit test). Future sub-plans introducing genuinely new reducer/game logic should still write real unit tests per the approach above; this exception is specific to pure mechanical dispatch-layer plumbing with no logic of its own to test.
